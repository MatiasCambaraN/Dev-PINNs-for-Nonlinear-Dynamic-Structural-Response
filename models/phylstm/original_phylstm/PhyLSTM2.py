...
with tf.device('/device:GPU:0'):
    ...
    Loss_u, Loss_udot, Loss_g, Loss_ut_c, Loss_e1, Loss, Loss_val, best_loss = model.train(num_epochs=5000, learning_rate=1e-3, bfgs=0)
    ...
    # Results
    ag_data = np.concatenate([mat['input_tf'], mat['input_pred_tf']])  # ag, ad, av
    u_data = np.concatenate([mat['target_X_tf'], mat['target_pred_X_tf']])
    ut_data = np.concatenate([mat['target_Xd_tf'], mat['target_pred_Xd_tf']])
    utt_data = np.concatenate([mat['target_Xdd_tf'], mat['target_pred_Xdd_tf']])
    ag_data = ag_data.reshape([ag_data.shape[0], ag_data.shape[1], 1])
    u_data = u_data.reshape([u_data.shape[0], u_data.shape[1], 1])
    ut_data = ut_data.reshape([ut_data.shape[0], ut_data.shape[1], 1])
    utt_data = utt_data.reshape([utt_data.shape[0], utt_data.shape[1], 1])
...
scipy.io.savemat(dataDir + 'results/results_PhyLSTM2.mat',
