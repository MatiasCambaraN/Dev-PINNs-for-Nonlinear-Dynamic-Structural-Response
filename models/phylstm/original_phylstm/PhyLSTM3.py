...
if __name__ == "__main__": 
    ...
    # Ruta al directorio actual del script (es decir: PhyLSTM-master/model/)
    current_dir = os.path.dirname(os.path.abspath(__file__))

    # Ruta a la carpeta raíz del proyecto (subiendo un nivel)
    project_root = os.path.abspath(os.path.join(current_dir, '..'))

    # Ruta a la carpeta /data
    dataDir = os.path.join(project_root, 'data')

    # Ruta al archivo .mat específico
    mat_path = os.path.join(dataDir, 'data_boucwen.mat')
...
with tf.device('/device:GPU:0'):
    ...
    # Training
    model = DeepPhyLSTM(eta_train, eta_t_train, g_train, ag_train, ag_c, lift, Phi_t)
    ...
    for ii in range(len(yt_pred_ref)):
        plt.figure()
    ...
    for ii in range(len(ytt_pred_ref)):
        plt.figure()
    ...
    for ii in range(len(g_pred_ref)):
        plt.figure()
...
scipy.io.savemat(dataDir + 'results_PhyLSTM3.mat',
